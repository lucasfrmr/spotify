    ...
    ws.on('message', async (message) => {
        ...
        switch (data.action) {
            ...
            case 'stopApp':
                ...
                break;
            // case 'requestAccess':
            //     console.log('requestAccess for username:', data.alias);
            //     const user = await db.collection('users').findOne({ username: data.alias });
            //     if (!user) {
            //         // Create a new record
            //         // const newUser = { username: data.username };
            //         createUser(data.alias, data.csid, ws);
            //         console.log('New user record created:', data.username);
            //     }
            //     if (user) {
            //         ws.send(JSON.stringify({ type: 'requestAccess-success', message: 'Access request successful' }));
            //     } else {
            //         ws.send(JSON.stringify({ type: 'requestAccess-error', message: 'Invalid username', username: data.username }));
            //     }
            //     break;
            case 'allowAccess':
                ...
                if (updateResult.matchedCount === 0) {
                    ...
                    console.log('User already had access:', data.username);
                    // Handle scenario when user already had access
                }
                break;
                
            case 'removeAccess':
                // set user grantedAccess to false based on username
            ...
            case 'search':
                ...
                const searchResults = await searchSpotify(data.query, data.username, data.csid, ws);
                ws.send(JSON.stringify({ type: 'search-results', results: searchResults }));
                break;
            case 'add-to-queue':
                console.log('add-to-queue ' + data.trackUri + ' ' + data.csid);
                await addToUserQueue(data.trackUri, data.username, data.csid, ws);
...
async function searchSpotify(query, username, csid, ws) {
...
async function addToUserQueue(trackUri, username, csid, ws) {
    console.log('running addToUserQueue() csid is:', username);
...
}, interval);import express from 'express';
import session from 'express-session';
import { MongoClient, ServerApiVersion } from "mongodb";
import fs from 'fs';
import { WebSocketServer } from 'ws';
