...
async function addSongToQueue(username, trackUri, trackName, artistName, duration_ms, ws) {
    ...
    try {
        ...
        wss.clients.forEach(async (client) => {
            const virtualQueue = await getVirtualQueue();
            client.send(JSON.stringify({ type: 'queue-data', queueItems: virtualQueue }));
        });
...
let currentSpotifyQueue;
async function updateSpotifyQueue() {
    const nextSong = (await getVirtualQueue())[0];
    await addSongToSpotifyQueue(nextSong.song, nextSong.username);
    currentSpotifyQueue = nextSong;
    wss.clients.forEach(async (client) => {
        const virtualQueue = await getVirtualQueue();
        client.send(JSON.stringify({ type: 'queue-data', queueItems: virtualQueue }));
    });
...
async function getCurrentPlayingTrack(access_token) {
    ...
    try {
        ...
        await db.collection('users').updateOne(
            { 'queuedSongs.trackUri': body.item.uri },
            { $set: { 'queuedSongs.$.played': true, 'queuedSongs.$.datePlayed': new Date() } }
        );

        // console.log('currently playing track:', body.item.id);
        if(!currentSpotifyQueue){
            updateSpotifyQueue();
            console.log('running');
        }
        // console.log('response is:', body.item.uri);
        // console.log('currentSpotifyQueue:', currentSpotifyQueue.song.trackUri);
        if(body.item.uri === currentSpotifyQueue.song.trackUri){
            updateSpotifyQueue();
        } else {
        }
