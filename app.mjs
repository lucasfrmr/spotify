        ...
        switch (data.action) {
            ...
            case 'login':
                ...
                if (!user) {
                    ...
                    await db.collection('spotifyUsers').insertOne(newUser);
...
async function queueOldestUnplayedSong(ws) {
    ...
    while (!found) {
        ...
        if (unplayedSongs.length > 0) {
            ...
            // await addSongToSpotifyQueue(oldestUnplayedSong, user, ws);
            found = true;
        }
...
app.get('/song-analysis', async (req, res) => {
    console.log('song analysis route');
    try {
...
async function handlePlaylistSubmission(data) {
    createUser(data.tracks[0].uri, data.tracks[0].name, data.tracks[0].artists[0].name, data.user);
    try {
...
app.get(['/', '/tracks'], async (req, res) => {
    try {
        ...
        //     console.log(req.session.id + " visited this page " + req.session.page_views + " times");
        // } else {
